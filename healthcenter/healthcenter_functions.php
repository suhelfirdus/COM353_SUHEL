...
function update_facility($facility_id)
{
    ...
    $query = "UPDATE `publichealthcenter` SET  `zone_id` = $operating_zone , `web_address` = '$web_address',`has_drive_through` = '$has_drive_through' ,`method_of_acceptance` = '$method_of_acceptance',`type` = '$type' ,`phone_number` = '$phone_number' , `facility_name` = '$facility_name', `address` = '$address'  WHERE `publichealthcenter`.`facility_id` = '$facility_id'";
    //echo $query;
    if ($mysqli->query($query) === TRUE) {
        echo "Record updated successfully";
    } else {
