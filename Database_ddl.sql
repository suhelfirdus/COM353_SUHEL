...
select `r`.`region_id`                                                                         AS `region_id`,
       ifnull(`qfc353_4`.`a`.`alert_id`, 0)                                                    AS `alert_id`,
       `r`.`region_name`                                                                       AS `region_name`,
       ifnull(`qfc353_4`.`a`.`alert_desc`, 'Level-1: Green - Vigilance')                       AS `alert_desc`,
...
create definer = qfc353_4@`172.16.0.0/255.240.0.0` view cities_det_view as
...
select `p`.`person_id`                                                                    AS `person_id`,
       ...
       (select `qfc353_4`.`publichealthcenter`.`facility_name`
        from `qfc353_4`.`publichealthcenter`
        where (`qfc353_4`.`publichealthcenter`.`facility_id` = `p`.`health_facility_id`)) AS `facility_name`,
       `p`.`related_person_no`                                                            AS `related_person_no`,
       `p`.`gender`                                                                       AS `gender`,
       ...
       (select `qfc353_4`.`region`.`region_name`
        ...
        where (`qfc353_4`.`region`.`region_id` = `a`.`region_id`))                        AS `region_name`,
       `a`.`city_id`                                                                      AS `city_id`,
       (select concat(`qfc353_4`.`city`.`city_name`, '-', `qfc353_4`.`city`.`province`)
        from `qfc353_4`.`city`
        where (`qfc353_4`.`city`.`city_id` = `a`.`city_id`))                              AS `city_name`,
       `a`.`postal_code`                                                                  AS `postal_code`,
...
create definer = qfc353_4@`172.16.0.0/255.240.0.0` view recommendations_det_view as
...
create definer = qfc353_4@`172.16.0.0/255.240.0.0` view regionwise_report_count as
select `a`.`region_id` AS `region_id`, `a`.`resultcount` AS `Positive_Count`, `b`.`resultcount` AS `Negative_Count`
from (select `qfc353_4`.`diagnostic_det_view`.`region_id` AS `region_id`, count(0) AS `resultcount`
      from `qfc353_4`.`diagnostic_det_view`
      where (`qfc353_4`.`diagnostic_det_view`.`result` = 'Positive')
      group by `qfc353_4`.`diagnostic_det_view`.`region_id`) `a`
         join (select `qfc353_4`.`diagnostic_det_view`.`region_id` AS `region_id`, count(0) AS `resultcount`
               from `qfc353_4`.`diagnostic_det_view`
               where (`qfc353_4`.`diagnostic_det_view`.`result` = 'Negative')
               group by `qfc353_4`.`diagnostic_det_view`.`region_id`) `b`
where (`a`.`region_id` = `b`.`region_id`);

create definer = qfc353_4@`172.16.0.0/255.240.0.0` view symptoms_det_table_view as
select `fad`.`person_id`     AS `person_id`,
       `fad`.`date_reported` AS `date_reported`,
       `fad`.`time_reported` AS `time_reported`,
       `fad`.`symptoms`      AS `symptoms`,
       `dfa`.`body_temp`     AS `body_temp`
from `qfc353_4`.`followup_details` `fad`
         join `qfc353_4`.`daily_follow_up` `dfa`
where ((`dfa`.`person_id` = `fad`.`person_id`) and (`dfa`.`date_reported` = `fad`.`date_reported`))
order by `dfa`.`date_reported`, `dfa`.`time_reported`;
